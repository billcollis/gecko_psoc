...
/* functiongen vars */
volatile FGen_Specifications_Struct sinegen_specs;
volatile FGen_Enable_Struct sinegen_enablestate;
volatile uint16 sinegen_sample_size = SINEGEN_SAMPLE_SIZE_1023; //initialised for 1kHz 

//try to avoid externs - use setters/getters instead
/* keypad vars */
...
void sinegen_init(){
    //load WaveDAC with ram based settings and 1kHz
    new_wavedac_setting(SINEGEN_SAMPLE_SIZE_1023, SINEGEN_INIT_FREQ);
}
...
void sinegen_display_labels(){
    ...
    lowerdisplay_clear();
    ssd_print_text(MT_ONOFF,MENU_X,SINEGEN_OUTPUT_Y);
    ssd_print_text(MT_FREQ,MENU_X,SINEGEN_FREQ_Y);
    ssd_print_text(MT_PERIOD,MENU_X,SINEGEN_PERIOD_Y);
    ssd_print_text(MT_AMPL,MENU_X,SINEGEN_AMPLITUDE_Y);
    ssd_print_text(MT_OFFSET,MENU_X,SINEGEN_OFFSET_Y);
    ssd_print_text(MT_OFFSET,MENU_X,SINEGEN_OFFSET_Y);
    ssd_print_text(MT_EXIT,MENU_X,SINEGEN_EXIT_Y);
    display_update();
} 


void sinegen_display_values(void){
    char text[15];
    if (sinegen_enablestate.enable == 0u){
        ssd_print_text("Off",SINEGEN_CENTER_X,SINEGEN_OUTPUT_Y);
    }
    else{
        ssd_print_text("On ",SINEGEN_CENTER_X,SINEGEN_OUTPUT_Y);
    }
       
    sprintfcomma(sinegen_specs.frequency,text,1,"Hz");
    ssd_print_text(text,SINEGEN_CENTER_X,SINEGEN_FREQ_Y);
    
    sprintfcomma(sinegen_specs.period,text,1,"ms");
    ssd_print_text(text,SINEGEN_CENTER_X,SINEGEN_PERIOD_Y);
        
    sprintfcomma(sinegen_specs.amplitude,text,1,"V");
    ssd_print_text(text,SINEGEN_CENTER_X,SINEGEN_AMPLITUDE_Y);
    
    sprintfcomma(sinegen_specs.offset,text,1,"V");
    ssd_print_text(text,SINEGEN_CENTER_X,SINEGEN_OFFSET_Y);
...
void sinegen_display(){
    sinegen_display_labels();
    sinegen_display_values();
    button=NONE;
}
void sendFreqToDDS_WaveDAC(uint32 freq){
    //freq is in milli Hz
    uint16 samples = 0; //SINEGEN_SAMPLE_SIZE_1023; //default for 1kHz
    ...
    /4000           500Hz
    */
    if (freq > SINEGEN_MAX_FREQ)
    {
        freq = SINEGEN_MAX_FREQ;
    }
    if (freq > SINEGEN_CUTOVER_1){ //up to 75kHz
        samples = SINEGEN_SAMPLE_SIZE_31;
    }else if (freq > SINEGEN_CUTOVER_2){ //up to 35kHz
        samples = SINEGEN_SAMPLE_SIZE_63;
    }else if (freq >  SINEGEN_CUTOVER_3){ //up to 18kHz
        samples = SINEGEN_SAMPLE_SIZE_127;
    }else if (freq >   SINEGEN_CUTOVER_4){ //up to 9kHz
        samples = SINEGEN_SAMPLE_SIZE_255;
    }else if (freq >   SINEGEN_CUTOVER_5){ //up to 4.5kHz
        samples =  SINEGEN_SAMPLE_SIZE_511;
    }else { //up to 2.2kHz
        samples =  SINEGEN_SAMPLE_SIZE_1023; 
    }

    dds_freq = freq * samples / 1000 ;
    
    //only change the wavdac if new range is required
    if (samples == sinegen_sample_size){
...
void new_wavedac_setting(uint16 samples, uint32 dds_freq){
        WaveDAC_Stop();
    if (SINEGEN_SAMPLE_SIZE_31 == samples){ //up to 75kHz
        WaveDAC_Wave1Setup(WaveDAC_wave31, SINEGEN_SAMPLE_SIZE_31) ;
    }else if (SINEGEN_SAMPLE_SIZE_63 == samples){ //up to 35kHz
        WaveDAC_Wave1Setup(WaveDAC_wave63, SINEGEN_SAMPLE_SIZE_63) ;
    }else if (SINEGEN_SAMPLE_SIZE_127 == samples){ //up to 18kHz
        WaveDAC_Wave1Setup(WaveDAC_wave127, SINEGEN_SAMPLE_SIZE_127) ;
    }else if (SINEGEN_SAMPLE_SIZE_255 == samples){ //up to 9kHz
        WaveDAC_Wave1Setup(WaveDAC_wave255, SINEGEN_SAMPLE_SIZE_255) ;
    }else if (SINEGEN_SAMPLE_SIZE_511 == samples){ //up to 4.5kHz
        WaveDAC_Wave1Setup(WaveDAC_wave511, SINEGEN_SAMPLE_SIZE_511) ;
    }else { //up to 2.2kHz
        WaveDAC_Wave1Setup(WaveDAC_wave1023, SINEGEN_SAMPLE_SIZE_1023) ;
    }
    sinegen_sample_size = samples; //save new sample size
...
void underline(int8 digit, uint8 line){
    uint8 x =  SINEGEN_CENTER_X + digit * DIGIT_WIDTH; 
    gfx_drawLine (x , line + DIGIT_HEIGHT - 1, x + DIGIT_WIDTH , line + DIGIT_HEIGHT - 1, 1);
}
void remove_underline (uint8 line){
    gfx_drawLine (SINEGEN_CENTER_X , line + DIGIT_HEIGHT - 1, SINEGEN_CENTER_X + SINEGEN_RIGHT_DIGIT * DIGIT_WIDTH , line+ DIGIT_HEIGHT - 1, 0);
}


//the menu routines
void sinegen_enter(){
    sinegen_remove_indicators();
    ssd_print_text("*", MENU_LEFT, MAIN_MENU_Y);
    display_update();
}
void sinegen_onoff(){
    sinegen_remove_indicators();
    //set_menumode_submenu_navigate();
    ssd_print_text("-", SINEGEN_LEFT_X, SINEGEN_OUTPUT_Y);
    //only process a select button if in the state  -stops selection when naigating
    if (button == SELECT && currentstate == ST_SI_ONOFF){ //changes the output
        if (sinegen_enablestate.enable == 1u){ //on
            sinegen_off();
            sinegen_enablestate.enable = 0;
            ssd_print_text("Off",SINEGEN_CENTER_X,SINEGEN_OUTPUT_Y);
        }else{
            sinegen_on();
            sinegen_enablestate.enable = 1;
            ssd_print_text("On ",SINEGEN_CENTER_X,SINEGEN_OUTPUT_Y);
        }
    }
    display_update();
...
void sinegen_on(){
    ...
    DDS_Start();
    //DDS_Enable();
    sendFreqToDDS_WaveDAC(sinegen_specs.frequency);
}
void sinegen_freq(){
    sinegen_remove_indicators();
    ssd_print_text("-", SINEGEN_LEFT_X, SINEGEN_FREQ_Y);
    remove_underline(SINEGEN_FREQ_Y);
    display_update();
}
void sinegen_freq_edit(){
    char text[15];
    static int8 digit_x = SINEGEN_LEFT_DIGIT; //start at left hand side(0)
    ssd_print_text("*", SINEGEN_LEFT_X, SINEGEN_FREQ_Y);
    underline(digit_x, SINEGEN_FREQ_Y);
    if (button == LEFT){ //navigate through digits
        remove_underline(SINEGEN_FREQ_Y);
        display_update();
        --digit_x;
        if (digit_x < SINEGEN_LEFT_DIGIT){ //gone past left digit, move to last digit
            digit_x = SINEGEN_RIGHT_DIGIT;
        }
        if (digit_x == COMMA){
            digit_x --;
        }
        ...
        display_update();
        digit_x ++;
        if (digit_x > SINEGEN_RIGHT_DIGIT){
            digit_x = SINEGEN_LEFT_DIGIT;
        }
        if (digit_x == COMMA){
            digit_x ++;
        }
        ...
        sinegen_specs.frequency += get_freq_incr_decr(digit_x);
        if (sinegen_specs.frequency > SINEGEN_MAX_FREQ){
            sinegen_specs.frequency = SINEGEN_MAX_FREQ;
        }
        sprintfcomma(sinegen_specs.frequency,text,1,"Hz");
        ssd_print_text(text,SINEGEN_CENTER_X,SINEGEN_FREQ_Y);
        sendFreqToDDS_WaveDAC(sinegen_specs.frequency);
        ...
        sinegen_specs.frequency -= get_freq_incr_decr(digit_x);
        if (sinegen_specs.frequency > SINEGEN_MAX_FREQ){
            sinegen_specs.frequency = 1u;
        }
        if (sinegen_specs.frequency == 0){
            sinegen_specs.frequency = 1u;
        }
        sendFreqToDDS_WaveDAC(sinegen_specs.frequency);
        sprintfcomma(sinegen_specs.frequency,text,1,"Hz");
    ...
    //else{//button == SELECT
    //    //set_menumode_mainmenu_navigate(); 
    //    ssd_print_text("-", SINEGEN_LEFT_X, SINEGEN_FREQ_Y);
    //    remove_underline(SINEGEN_FREQ_Y);
    //    set_state(ST_SINEGEN); //finished setting go back to FG menu 
    //}  
    //work out period in uS
    double period = sinegen_specs.frequency;
    period = 1000000000 / period;
    if (period > 999999999){
        period = 999999999;
    }
    sinegen_specs.period = (uint32) period;
    sprintfcomma(sinegen_specs.period,text,1,"ms");
    ssd_print_text(text,SINEGEN_CENTER_X,SINEGEN_PERIOD_Y);
...
void sinegen_period(){
    sinegen_remove_indicators();
    ssd_print_text("-", SINEGEN_LEFT_X, SINEGEN_PERIOD_Y);
    display_update();
}
void sinegen_amplitude(){
    //char text[15];
    //static int8 digit_x = SINEGEN_LEFT_V_DIGIT; //start at digit 6
    sinegen_remove_indicators();
    /*
    if (is_submenu_navigatemode()){
        //switch to editting submenu
        if (button == SELECT && currentstate == ST_AMPL){
            ssd_print_text("*", SINEGEN_LEFT_X, SINEGEN_AMPLITUDE_Y);
            //set_menumode_submenu_edit();
        }
        else{
            ssd_print_text("-", SINEGEN_LEFT_X, SINEGEN_AMPLITUDE_Y);
        }
        ...
        if (button == LEFT){ //navigate through digits
            remove_underline(SINEGEN_AMPLITUDE_Y);
            display_update();
            --digit_x;
            if (digit_x < SINEGEN_LEFT_V_DIGIT){ //gone past left digit, move to last digit
                digit_x = SINEGEN_RIGHT_DIGIT;
            }
            if (digit_x == DECIMAL_POINT){
                digit_x --;
            ...
            sinegen_specs.amplitude += get_freq_incr_decr(digit_x);
            if (sinegen_specs.amplitude > SINEGEN_MAX_AMPL){
                sinegen_specs.amplitude = SINEGEN_MAX_AMPL;
            }
            sprintfcomma(sinegen_specs.frequency,text,1,"V");
            ssd_print_text(text,SINEGEN_CENTER_X,SINEGEN_AMPLITUDE_Y);
            //sendAmplToDDS_WaveDAC(sinegen_specs.amplitude);
        }
        else if (button == DOWN){//decrease current digit
            sinegen_specs.amplitude -= get_freq_incr_decr(digit_x);
            if (sinegen_specs.amplitude > SINEGEN_MAX_AMPL){
                sinegen_specs.amplitude = 1u;
            }
            if (sinegen_specs.amplitude == 0){
                sinegen_specs.amplitude = 1u;
            }
            //sendAmplToDDS_WaveDAC(sinegen_specs.amplitude);
            sprintfcomma(sinegen_specs.amplitude,text,1,"V");
        ...
        underline(digit_x, SINEGEN_AMPLITUDE_Y + DIGIT_HEIGHT - 1);
    }
    display_update();
    */
}
void sinegen_offset(){
    sinegen_remove_indicators();
    ssd_print_text("-", SINEGEN_LEFT_X, SINEGEN_OFFSET_Y);
    display_update();
}
void sinegen_back(){
    sinegen_remove_indicators();
    ssd_print_text("-", SINEGEN_LEFT_X, SINEGEN_EXIT_Y);
    display_update();
}
